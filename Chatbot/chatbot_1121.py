...
auth_token = "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"
# 모델 로드
pipe = StableDiffusionPipeline.from_pretrained(model_id, use_auth_token=auth_token).to("cuda")
...
from auto_order_system import *

memory = MemorySaver()
# 전역 변수로 QTextEdit 객체를 선언
global_text_edit = None  # 초기화 (전역 변수)
global m
match_auto_order_pattern = None
# 메모리 세팅 시 필요한 키들
memory_config = {
...
class ChatApp(QWidget):
    ...
    def generate_styled_image(self, prompt, style="vibrant"):
        styled_prompt = f"{prompt} with a {style} style"
    ...
    def send_message(self):
        ...
        if user_message:
            ...
            for focus in selected_focus:
                ...
                ### 검색 및 주문 접수 모드
                - 사용자가 검색 옵션을 선택하지 않으면, 기본적으로 웹 검색을 통해 정보를 제공합니다.
                - 사용자가 주문을 의도하는 발화를 할 경우, 검색 옵션 설정 없이 주문 접수 모드로 전환하여 주문을 받습니다.

                ### 자동 주문 모드
                관리자가 설정한 재고 데이터를 바탕으로 사용자의 구매 패턴을 분석하여 부족한 재고를 자동으로 주문하는 모드입니다. 
                - 이 모드는 사용자가 직접 주문을 하지 않아도 자동으로 발동할 수 있습니다.
                - 사용자가 재고 부족을 언급하거나, 특정 아이템이 재고 부족 상태일 경우, 해당 아이템의 자동 주문 처리가 진행됩니다.
                - 사용자가 "재고 부족", "이 제품이 품절이야?" 등의 발화를 하면 자동으로 이 모드가 활성화되어 부족한 재고를 주문하는 처리 과정을 시작합니다.
                - 이때, **`order_summary`**를 제공하여 주문 내용을 확인할 수 있습니다.
                
                **자동 주문 처리에 대한 답변 예시**:
                - 사용자가 "이 제품 재고가 부족하다고?"라고 물으면:
                  - "재고 부족 상태를 확인한 후, 자동으로 주문을 처리하겠습니다. 잠시만 기다려 주세요."
                  
                - 사용자가 "초코맛 아이스크림 재고가 부족하면 자동으로 주문할 거야?"라고 물으면:
                  - "네, 초코맛 아이스크림 재고가 부족하면 자동으로 주문 처리가 진행됩니다. 필요하신 양을 확인하여 주문을 진행하겠습니다."
                ...
                **최종 주문 확인 메시지**:
                ...
                auto_order_pattern = r".*(주문|재고 부족|품절|재고).*"
                match_auto_order_pattern = re.search(auto_order_pattern, user_message)
                ...
                if match_recommedation_pattern:
                ...
                elif match_auto_order_pattern:

                    # 텍스트 프롬프트를 디퓨전 모델로 변환
                    image = self.generate_image(prompt)

                    # 이미지를 Base64로 인코딩하여 응답에 포함
                    ...
                    print(fr'img_base64 :{img_base64}')

                    # 챗봇 응답
                    # return {
                    #     "messages": [
                    #         {"role": "system", "content": "이미지 생성 완료."},
                    #         {"role": "user", "content": prompt},
                    #         {"role": "assistant",
                    #          "content": f"생성된 이미지를 확인하세요:\n<img src='data:image/png;base64,{img_base64}'/>"}
                    #     ]
                    # }
                    result = {
                        "messages": [
                            ...
                            {"role": "assistant", "content": "주문 내용 확인 중..."},
                            {"role": "assistant", "content": f"생성된 이미지를 확인하세요:\n<img src='data:image/png;base64,{img_base64}'/>"}
                        ]
                    }
    ...
    {'role': 'assistant', 'focus': 'web', 'content': '바나나맛 아이스크림을 주문하셨습니다. 추가할 토핑은 무엇으로 하시겠습니까? (로투스, 레인보우, 오레오 중에서 선택해 주세요.)'}]
                ...
                for message in self_messages:
                    ...
                    if message['role'] == 'assistant':
                        ...
                        """
                            ...
                            재고가 부족한 상품에 대해만 자동으로 주문을 추가합니다.
                            """
                        auto_order()

                        # 주문 데이터 출력 (자동으로 생성된 주문 정보)
                    print(fr'df_orders : {df_orders}')
                ...
                if "messages" in result and len(result["messages"]) > 0:
                    ...
                    assistant_response = {
                        "role": "assistant",
                        "focus": focus,  # focus 정보를 추가
                        "content": assistant_response # 응답 내용 추가
                    }
                    print(f'send_message assistant_response: {assistant_response}')
                    responses.append((selected_focus, assistant_response))
